...
/etc/init.d/httpd -V | grep --color=never MPM; grep --color=never "KeepAlive " $HTTPD; egrep 'MaxClients|KeepAlive|MaxRequestsPerChild|Timeout|Servers|Threads|ServerLimit|MaxRequestWorkers|MaxConnectionsPerChild' $HTTPD; INCLUDES=$(cat /usr/local/apache/conf/includes/pre_virtualhost_global.conf); if [[ $? == 0  ]] ; then echo -e "\n$yellow/usr/local/apache/conf/includes/pre_virtualhost_global.conf:$nocolor\n \n$INCLUDES"; fi
